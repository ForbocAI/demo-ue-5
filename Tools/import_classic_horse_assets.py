...
HORSE_SOURCES = [
    ZipImportSource(
        ...
        "/Game/Map/Horses/ClassicHorse/SkeletalMesh",
    ),
    ZipImportSource(
        "ClassicHorse/1. Horse/Animations/InPlace/3. walk.fbx",
        "Horse_Walk.fbx",
        "/Game/Map/Horses/ClassicHorse/Animations",
    ),
    ZipImportSource(
    ...
    ZipImportSource(
        ...
        "/Game/Map/Horses/ClassicHorse/Textures/Horse",
    ),
    ZipImportSource(
...
RIDER_SOURCES = [
    ZipImportSource(
        ...
        "/Game/Map/Horses/ClassicHorse/Rider/SkeletalMesh",
    ),
    ZipImportSource(
        "ClassicHorse/2. Rider/Animations/InPlace/3. walk.fbx",
        "Rider_Walk.fbx",
        "/Game/Map/Horses/ClassicHorse/Rider/Animations",
    ),
    ZipImportSource(
    ...
    ZipImportSource(
        ...
        "Rider_Albedo.psd",
        "/Game/Map/Horses/ClassicHorse/Rider/Textures",
    ),
    ZipImportSource(
...
def main() -> int:
    if not SOURCE_ZIP.exists():
        unreal.log_error(f"MapHorseImport: missing {SOURCE_ZIP}")
        return 1

    sources = HORSE_SOURCES + RIDER_SOURCES
    extract_sources(sources)
    import_sources(sources)
    unreal.EditorAssetLibrary.save_directory("/Game/Map/Horses", True)
