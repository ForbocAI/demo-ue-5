...
function Find-DataJsonByKey {
  param([string] $Key)

  $Matches = @(
    Get-ChildItem -LiteralPath $DataRoot -Filter "*.json" -File -Recurse |
      Where-Object {
        try {
          $Document = Get-Content -LiteralPath $_.FullName -Raw | ConvertFrom-Json
          $Property = $Document.PSObject.Properties[$Key]
          ($null -ne $Property) -and ($Property.Value -is [pscustomobject])
        } catch {
          $false
        }
      }
  ...
  if ($Matches.Count -ne 1) {
    $Discovered = ($Matches | ForEach-Object { $_.FullName }) -join ", "
    throw "Expected exactly one data JSON document with top-level key '$Key'; found $($Matches.Count): $Discovered"
  }
...
$ProfilePath = Find-DataJsonByKey -Key "rendering_profile"
