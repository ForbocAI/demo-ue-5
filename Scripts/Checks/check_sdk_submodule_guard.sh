...
if [ "$ALLOW_GITLINK_CHANGE" -eq 0 ]; then
  if ! git -C "$PROJECT_ROOT" diff --quiet -- "$SDK_SUBMODULE"; then
    echo "Error: The SDK submodule pointer changed outside bash Scripts/SDK/update-sdk.sh."
    print_sdk_policy
    exit 1
  fi

  if ! git -C "$PROJECT_ROOT" diff --cached --quiet -- "$SDK_SUBMODULE"; then
    echo "Error: Staged SDK submodule pointer changes must be created by bash Scripts/SDK/update-sdk.sh."
    print_sdk_policy
    exit 1
  fi
fi
