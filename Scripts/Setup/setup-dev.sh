...
if [ -d "$PROJECT_ROOT/Plugins/ForbocAI_SDK" ]; then
  ...
  FORBOC_ALLOW_SDK_SUBMODULE_UNLOCK=setup-dev \
    bash "$PROJECT_ROOT/Scripts/SDK/lock_sdk_submodule.sh" --unlock || true
fi
...
chmod +x "$PROJECT_ROOT/Scripts/Checks/check_sdk_submodule_guard.sh"
chmod +x "$PROJECT_ROOT/Scripts/SDK/lock_sdk_submodule.sh"
...
cat <<'MSG'
  2. Return here and run bash Scripts/SDK/update-sdk.sh
