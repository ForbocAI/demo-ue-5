...
SDK_ACL_HELPER="$PROJECT_ROOT/Scripts/SDK/sdk_submodule_acl.ps1"
...
case "$MODE" in
  --lock|--unlock)
    ;;
  --help|-h)
    echo "Usage: Scripts/SDK/lock_sdk_submodule.sh [--lock|--unlock]"
    print_sdk_policy
    exit 0
    ;;
  *)
...
if [ ! -d "$SDK_PATH" ]; then
  ...
  echo "First-time setup: bash Scripts/Setup/setup-dev.sh"
  exit 1
fi
...
if [ "$MODE" = "--unlock" ]; then
  case "${FORBOC_ALLOW_SDK_SUBMODULE_UNLOCK:-}" in
    ...
    *)
      ...
      echo "  bash Scripts/Setup/setup-dev.sh"
      echo "  bash Scripts/SDK/update-sdk.sh"
      exit 1
      ;;
  esac
...
bash "$PROJECT_ROOT/Scripts/Checks/check_sdk_submodule_guard.sh" --allow-gitlink-change
git_submodule_env -C "$SDK_PATH" config core.hooksPath "$SDK_HOOKS_PATH" || true
case "${FORBOC_ENFORCE_SDK_ACL:-0}" in
  1|true|TRUE|yes|YES)
    apply_windows_write_deny
    ...
    ;;
  *)
    echo "SDK submodule source guarded by hooks/status checks. Optional ACL helper: Scripts/SDK/sdk_submodule_acl.ps1."
    ;;
esac
print_sdk_policy
