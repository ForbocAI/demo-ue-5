...
// --- Profile color declaration + composer --------------------------------
// One composer owns FLinearColor assembly. Each color is expressed as its
// irreducible RGBA member declaration fed to that composer, so no subdomain
// re-hand-writes `FLinearColor(Profile.XxxR, ...G, ...B, ...A)`.
...
inline FLinearColor ProfileLinearColor(const FLevelRetroRenderProfile &Profile,
