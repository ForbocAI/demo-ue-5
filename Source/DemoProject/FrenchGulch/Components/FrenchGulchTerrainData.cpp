...
bool FFrenchGulchTerrainData::LoadFromContent() {
  SourcePath = FPaths::ProjectContentDir() /
               TEXT("FrenchGulch/Heightmaps/french_gulch_ca_terrain_65.csv");

  TArray<FString> Lines;
  if (!FFileHelper::LoadFileToStringArray(Lines, *SourcePath)) {
    UE_LOG(LogTemp, Warning, TEXT("FrenchGulch: Failed to load terrain CSV: %s"),
           *SourcePath);
    return false;
  }
  ...
  for (const FString &RawLine : Lines) {
    ...
    if (ParsedGridSize == 0) {
      ...
      UE_LOG(LogTemp, Warning,
             TEXT("FrenchGulch: Terrain row width mismatch in %s"), *SourcePath);
      return false;
    }
  ...
  if (ParsedGridSize <= 1 || ParsedElevations.Num() != ParsedGridSize * ParsedGridSize) {
    UE_LOG(LogTemp, Warning, TEXT("FrenchGulch: Invalid terrain CSV grid: %s"),
           *SourcePath);
    return false;
  }
  ...
  UE_LOG(LogTemp, Display,
         TEXT("FrenchGulch: Loaded %dx%d terrain CSV from %s (%.1fm..%.1fm)"),
         GridSize, GridSize, *SourcePath, MinElevationMeters,
         MaxElevationMeters);
  return true;
}

bool FFrenchGulchTerrainData::IsLoaded() const {
  return GridSize > 1 && ElevationsMeters.Num() == GridSize * GridSize;
}

float FFrenchGulchTerrainData::GetElevationMetersAt(int32 Row,
                                                    int32 Column) const {
  if (!IsLoaded()) {
    return 0.0f;
  }
...
float FFrenchGulchTerrainData::GetTerrainZAtWorld(float EastWest,
...
FVector FFrenchGulchTerrainData::ToWorld(float EastWest, float NorthSouth,
                                         float HeightOffset) const {
  return FVector(EastWest, NorthSouth,
                 GetTerrainZAtWorld(EastWest, NorthSouth) + HeightOffset);
}

int32 FFrenchGulchTerrainData::GetGridSize() const { return GridSize; }

float FFrenchGulchTerrainData::GetMinElevationMeters() const {
  return MinElevationMeters;
}

float FFrenchGulchTerrainData::GetMaxElevationMeters() const {
  return MaxElevationMeters;
}

FString FFrenchGulchTerrainData::GetSourcePath() const { return SourcePath; }

float FFrenchGulchTerrainData::SampleElevationMeters(float EastWest,
                                                     float NorthSouth) const {
